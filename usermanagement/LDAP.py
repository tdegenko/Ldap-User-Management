...
class LDAPConn:
    ...
    def find_domain(self):
        target_info = self.ldap.search_s(
            self.server.base_domain_dn_str,
            ldap.SCOPE_SUBTREE,
            "(sambaDomainName=*)",
            attrlist=["dn", "sambaDomainName", "sambaSID"],
        )
    ...
    def next_uid(self, domain, uid_type="uidNumber"):
        ...
        modlist = ldap.modlist.modifyModlist(
            mod_old, {uid_type: [str(uid + 1).encode("utf8")]}
        )
...
class LDAPServer:
    ...
    def __init__(
        ...
        group_base=None,
        computer_base=None,
        guest_group="Domain Guests",
        user_group="Domain Users",
        computer_group="Domain Computers",
    ):
        self.url = url
    ...
    def __eq__(self, other):
        if not isinstance(other, LDAPServer):
            return False
        return (
            self.base_domain_dn_str == other.base_domain_dn_str
            and self.user_group == other.user_group
            and self.guest_group == other.guest_group
            and self.computer_group == self.computer_group
        )
